...
/*
This function calls function to get the user's query and then searches the query using goroutines
*/
func runScraperWithGoRoutines() {
	...
	goR, sites, inputIsInvalid := processWebsiteChoice(websiteChoice)
	if inputIsInvalid {
		return
	}
	...
	if goR == 1 {
		...
		// For each site, start a new routine and run them simultaneously
		for _, site := range sites {
			wg.Add(1) // Add a new goroutine
			go func(site string) {
				defer wg.Done() // Search is done
				if runQuery(functionName, description, language, site) == 1 {
					fmt.Println("Time to process query with Go routines: ", time.Since(startTime))
					return
				}
			}(site)
		}
		...
		// If not searching multiple sites, no need for the goroutine
		for _, site := range sites {
			if runQuery(functionName, description, language, site) == 1 {
				fmt.Println("Time to process query with Go routines: ", time.Since(startTime))
				return
